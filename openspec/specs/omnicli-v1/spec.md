...
> Current behavior of OmniCLI. Synced from `changes/archive/2026-04-25-omnicli-v1/spec.md` and `changes/mcp-integration/specs/omnicli-v1/spec.md` (2026-04-28).
...
The status bar MUST show the current activity state: "Searching", "Thinking", or "Executing".

#### Scenario: Activity state transitions
- GIVEN the agent is processing a request
- WHEN the agent invokes a search tool
- THEN the indicator MUST show "Searching"
- WHEN the agent is waiting for LLM response
- THEN the indicator MUST show "Thinking"
- WHEN the agent executes a shell command
- THEN the indicator MUST show "Executing"
